...
extern const unsigned int Runner_dll_len;
extern unsigned char Runner_dll[];
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	bstr_t bstrAssemblyName("TestClass");
	_AssemblyPtr spAssembly = NULL;

	// The .NET class to instantiate.
	bstr_t bstrClassName("TestClass");
	...
	// (Option 1) Load it from disk - usefully when debugging the Loader app (you'll have to copy the DLL into the same directory as the exe)
	// hr = spDefaultAppDomain->Load_2(bstrAssemblyName, &spAssembly);

	// (Option 2) Load the assembly from memory
	SAFEARRAYBOUND bounds[1];
	bounds[0].cElements = Runner_dll_len;
	bounds[0].lLbound = 0;

	SAFEARRAY* arr = SafeArrayCreate(VT_UI1, 1, bounds);
	SafeArrayLock(arr);
	memcpy(arr->pvData, Runner_dll, Runner_dll_len);
	SafeArrayUnlock(arr);
	...
	wchar_t* argument = L"calc.exe";

	InvokeMethod(spType, L"RunProcess", argument);
...
void InvokeMethod(_TypePtr spType, wchar_t* method, wchar_t* command)
{
	...
	if (FAILED(hr))
	{
		wprintf(L"Failed to invoke RunProcess w/hr 0x%08lx\n", hr);
		return;
	}
	else
	{
